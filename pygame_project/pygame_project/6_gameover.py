...
stage_height = stage_size[1] # 스테이지의 높이를 이용해 캐릭터를 그 위에 올리기 위해
...
while running:
    ...
    # 공 위치 정의
    for ball_idx, ball_val in enumerate(balls):  # 값과 인덱스까지 모두 표시할 수 있음 ball_idx는 인덱스값(0,1,2) ball_vall은 값
    ...
    for ball_idx, ball_val in enumerate(balls):  # 값과 인덱스까지 모두 표시할 수 있음 ball_idx는 인덱스값(0,1,2) ball_vall은 값
        ...
        break # 안쪽 for 문에서 break 를 만나면 여기로 진입 가능, 2중 for 문을 한번
    ...
    # 충돌된 공 or 무기 없애기
    if ball_to_remove > -1:
        del balls[ball_to_remove] # del : 리스트에 해당하는 위치 값 지우기
        ball_to_remove = -1
